...
filter_by_size_range() {
	...
	awk -v max_value="$max_value" '($5 == "P" || $5 == "I") && ($4 >= 35 && $4 <= max_value) {print $0}' "$merged_fq" | sort -k1,1 --temporary-directory="${output_dir}" --parallel=6 --buffer-size=25% --compress-program=gzip --batch-size=1000 > "$out_filtered_fq"
	check_file_exists "$out_filtered_fq"
}

# Main processing loop for each sample
for sample_id in $(cut -f 1 "$samples_file" | grep -v "sample_name" ); do
	...
	bbmerge.sh in="$in_fq1" in2="$in_fq2" outinsert="$out_merged_fq"
